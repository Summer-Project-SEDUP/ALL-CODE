...
get_interval <- function(v_bin_data, binfile, start, end, calib, freq){
  ...
  window_size <- round(10 * 60 * freq)  # 10-minute window size
  step_size <- round(freq * 60)  # 1-minute step size
  
  non_wear_hours = mean(rollapply(day_data, window_size, non_wear_window, by = step_size, by.column = FALSE)) * n_hours
...
SedUp <- function(y, fs, option) {
  ...
  sdv <- apply(matrix(y, nrow = round(fs)), 2, sd)
  window1 <- round(nSec * constant * fs / 2)
  ...
  for (i in seq(1, length(sdv), by = constant)) {
    j1   <- j1 + 1
...
compute_for_all("D:/Downloads/in", "D:/Downloads/out/recal-window")

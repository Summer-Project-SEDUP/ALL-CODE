...
interpolate_data <- function(data, original_freq, new_freq) {
  original_time_stamps <- seq(from = 0, by = 1/original_freq, length.out = nrow(data))
  new_time_stamps <- seq(from = 0, by = 1/new_freq, length.out = round(nrow(data) * (new_freq / original_freq)))
  
  # Interpolate each column separately
  interpolated_data <- sapply(1:ncol(data), function(i) {
    approx(x = original_time_stamps, y = data[, i], xout = new_time_stamps)$y
  })
  
  colnames(interpolated_data) <- colnames(data)
  return(as.data.frame(interpolated_data))
}

get_interval <- function(v_bin_data, binfile, start, end, calib, original_freq, new_freq) {
  day_data = read.bin(binfile, start = start, end = end)$data.out
  day_data = cbind(day_data[, 1], ((calib[1:3] * day_data[, c('x', 'y', 'z')]) + calib[4:6]), day_data[, 7])
  interpolated_data <- interpolate_data(day_data, original_freq, new_freq)
  n_hours = (nrow(interpolated_data) / new_freq / 3600)
  non_wear_hours = mean(rollapply(interpolated_data, (10 * 60 * new_freq), non_wear_window, by = (new_freq * 60), by.column = FALSE), na.rm = TRUE) * n_hours
  date = as.Date(as.POSIXct(interpolated_data[nrow(interpolated_data) / 2, 1], origin = "1970-01-01"))
  day_of_week = weekdays(date)
  return(list("DataFrame" = interpolated_data, "Date" = date, "WeekDay" = day_of_week, "DayNo" = strsplit(start, " ")[[1]][1], "NonWear" = non_wear_hours))
}
...
non_wear_window <- function(window) {
  ...
  temp_in_window1 = mean(window1[, 5], na.rm = TRUE)
  temp_in_window2 = mean(window2[, 5], na.rm = TRUE)
  
  stds_in_window1 = apply(window1[, c('x', 'y', 'z')], 2, sd, na.rm = TRUE)
  min_in_window1 = apply(window1[, c('x', 'y', 'z')], 2, min, na.rm = TRUE)
  max_in_window1 = apply(window1[, c('x', 'y', 'z')], 2, max, na.rm = TRUE)
  ...
  if (anyNA(temp_in_window1) || anyNA(temp_in_window2) || anyNA(stationary)) {
    print("NA values found in window metrics")
    return(NA)
  }
...
SedUp <- function(y, fs, option) {
  ...
  sdv <- apply(matrix(y, nrow = fs), 2, sd, na.rm = TRUE)
...
extract_metrics <- function(raw_bout, freq) {
  ...
  intensity = mean(raw_bout[,"ENMO"], na.rm = TRUE)
  timestamp = strftime(format(as.POSIXct(raw_bout[1, 1], origin = "1970-01-01"), tz = "UTC"), format = "%H:%M:%S")
  duration = nrow(raw_bout) / freq
  percentiles = as.vector((quantile(raw_bout[,'ENMO'], probs = seq(0, 1, 0.05), na.rm = TRUE)))
  sd = sd(raw_bout[,"ENMO"], na.rm = TRUE)
...
wind_SD <- function(vect) {
  top = as.numeric(quantile(vect, probs = c(0.95), na.rm = TRUE))
  bottom = as.numeric(quantile(vect, probs = c(0.05), na.rm = TRUE))
  wind_vect = pmin(pmax(vect, bottom), top)
  return(sd(wind_vect, na.rm = TRUE))
}
...
compute_one_person <- function(file_name, binfolder, outfolder) {
  ...
  original_freq = as.numeric(as.character(v_bin_data$header['Measurement_Frequency', 1]))
  new_freq = round(original_freq)
  print("Original freq:")
  print(original_freq)
  print("New freq:")
  print(new_freq)
  ...
  for (i in (1:(length(filenames) - 1))) {
    ...
    D1 = get_interval(v_bin_data, binfile, start = start, end = end, calib = calib, original_freq = original_freq, new_freq = new_freq)
    if (nrow(D1$DataFrame) > 3000) {
      bouts = compute_bouts(D1$DataFrame, new_freq)
      ENMO_tab = add_ENMO(D1$DataFrame)
      raw_bouts = get_raw_data(ENMO_tab, bouts, new_freq)
      if (nrow(bouts) > 1) {
        list_of_bouts = lapply(raw_bouts, extract_metrics, freq = new_freq)
      ...
      if (nrow(bouts) == 1) {
        table_of_bouts = extract_metrics(ENMO_tab, new_freq)
...
compute_for_all("D:/Downloads/in", "D:/Downloads/out/interpol")

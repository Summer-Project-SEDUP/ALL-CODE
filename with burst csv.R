...
# Sedentary and upright posture detection
...
compute_one_person <- function(file_name, binfolder, outfolder) {
  ...
  dir.create(specific_out_folder, recursive = TRUE, showWarnings = FALSE)
...
# Function to calculate the novel burstiness measure A_n(r)
calculate_A_n <- function(r, n) {
...
# Function to compute burstiness measures for a given file
compute_burstiness <- function(file_path, output_path, original_binfile) {
  # Read the CSV file
  ...
  # Remove the first row of each group since it has no valid time difference
  filtered_data <- filtered_data %>% 
    group_by(Date) %>% 
    filter(!is.na(Time_Diff)) %>%
    ungroup()
  
  # Calculate daily means and standard deviations of interevent times
  daily_stats <- filtered_data %>%
    group_by(Date) %>%
    summarise(
      Mean_Interevent = mean(Time_Diff),
      SD_Interevent = sd(Time_Diff),
      n = n(),  # Number of observations
      .groups = 'drop'
    )
  
  # Calculate burstiness parameter for each day
  daily_stats <- daily_stats %>%
    mutate(
      Burstiness = (SD_Interevent - Mean_Interevent) / (SD_Interevent + Mean_Interevent),
      r = SD_Interevent / Mean_Interevent,
      A_n = calculate_A_n(r, n),
      File = original_binfile
    )
  
  # Write the burstiness measures to a CSV file
  output_file <- file.path(output_path, paste0(original_binfile, "_burstiness_measures.csv"))
  write.csv(daily_stats, output_file, row.names = FALSE)
  
  print(paste("Burstiness measures saved to:", output_file))
  
  return(daily_stats)  # Return daily_stats for combining later
}

# Function to process all participants and compute burstiness measures
compute_for_all_with_burstiness <- function(binfolder, outfolder) {
  # Get all bin files
  all_binfiles = list.files(binfolder, pattern = "*.bin", full.names = TRUE)
  
  # Output folder for burstiness measures
  burstiness_output_folder <- file.path(outfolder, "burstiness_measures")
  dir.create(burstiness_output_folder, showWarnings = FALSE, recursive = TRUE)
  
  # List to hold all burstiness data for combining
  all_burstiness_data <- list()
  
  # Compute burstiness for each file
  lapply(all_binfiles, function(binfile) {
    # Extract the file name without the .bin extension
    file_name_without_bin <- sub("\\.bin$", "", basename(binfile))
    
    # Compute the initial data processing
    compute_one_person(basename(binfile), binfolder, outfolder)
    
    # Path to the processed all_days.csv file
    specific_out_folder <- file.path(outfolder, file_name_without_bin)
    all_days_file <- file.path(specific_out_folder, "all_days.csv")
    
    # Check if the file exists before computing burstiness
    if (file.exists(all_days_file)) {
      burstiness_data <- compute_burstiness(all_days_file, burstiness_output_folder, file_name_without_bin)
      all_burstiness_data[[file_name_without_bin]] <- burstiness_data  # Store the burstiness data
    } else {
      print(paste("Processed data file does not exist for:", file_name_without_bin))
    }
  })
  
  # Combine all burstiness data into one data frame
  combined_burstiness <- bind_rows(all_burstiness_data)
  
  # Write combined burstiness measures to a CSV file
  combined_output_file <- file.path(outfolder, "combined_burstiness_measures.csv")
  write.csv(combined_burstiness, combined_output_file, row.names = FALSE)
  
  print(paste("Combined burstiness measures saved to:", combined_output_file))
}

# Run the function for all participants
compute_for_all_with_burstiness("D:/Downloads/p004", "D:/Downloads/out/004")

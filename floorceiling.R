...
get_interval <- function(v_bin_data, binfile, start, end, calib, freq) {
  ...
  non_wear_hours = mean(zoo::rollapply(day_data, (10 * 60 * freq), non_wear_window, by = (freq * 60), by.column = FALSE, fill = NA, align = 'right')) * n_hours
  date = as.Date(as.POSIXct(day_data[floor(nrow(day_data)/2), 1], origin="1970-01-01"))
...
non_wear_window <- function(window) {
  ...
  if ((sum(stationary) >= 2) & (temp_in_window1 < t0)) {
    ...
    if (temp_in_window1 > temp_in_window2) {
      status = 1
    } else if (temp_in_window1 < temp_in_window2) {
      status = 0
    }
  }
  return(status)
...
SedUp <- function(y, fs, option) {
  ...
  sdv <- apply(matrix(y, nrow = floor(fs)), 2, sd)
  ...
  for (i in seq(from = 1, to = length(sdv), by = constant)) {
    j1 <- j1 + 1
    i1_1 <- max(1, floor((i - 1) * fs + 1 - window1))
    i1_2 <- min(length(y), ceiling((i - 1) * fs + 1 + constant * fs + window1))
    b <- i1_1:i1_2
    medacc[j1] <- median(y[b], na.rm = TRUE)
    j2 <- j2 + 1
    i2_1 <- max(1, floor((i - 1) - window2))
    i2_2 <- min(length(sdv), ceiling((i - 1) + 1 + constant + window2))
    b <- i2_1:i2_2
    medsd[j2] <- median(sdv[b], na.rm = TRUE)
  }
  ...
  standing <- rep(standing, each = constant * floor(fs))
  return(standing)
}

get_raw_data <- function(D, bouts, freq) {
  apply(bouts, 1, function(x) {
    start_index <- max(1, floor(x[1]))
    end_index <- min(nrow(D), ceiling(x[2]))
...
compute_bouts <- function(D, freq) {
  X = SedUp(D[,3], freq, 1)
  rle_bouts = rle(X) 
  start = cumsum(c(0, head(rle_bouts$lengths, -1))) + 1
...
compute_for_all("D:/Downloads/in", "D:/Downloads/out/floor")

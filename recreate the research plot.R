...
# Function to check non-wear window
...
# Function to compute sedentary and upright metrics
SedUp <- function(y, fs, option) {
  ...
  return(list(medacc = rep(medacc, each = constant * fs), medsd = rep(medsd, each = constant * fs), standing = standing))
}
...
# Function to compute Windsorized SD
...
compute_bouts <- function(D, freq) {
  result <- SedUp(D[, 3], freq, 1)
  X <- result$standing
  rle_bouts <- rle(X)
  start <- head(cumsum(c(0, rle_bouts$lengths)), -1)
  end <- cumsum(rle_bouts$lengths)
  medacc <- result$medacc[start + 1]
  medsd <- result$medsd[start + 1]
  return(data.frame(start = start, end = end, lab = rle_bouts$values, medacc = medacc, medsd = medsd))
}

# Function to create ghost table
...
compute_one_person <- function(file_name, binfolder, outfolder) {
  ...
  for (i in (1:(length(filenames) - 1))) {
    ...
    if (nrow(D1$DataFrame) > 3000) {
      ...
      if (nrow(bouts) > 1) {
        ...
        table_of_bouts <- cbind(table_of_bouts, "medacc" = bouts$medacc)
        table_of_bouts <- cbind(table_of_bouts, "medsd" = bouts$medsd)
      }
      if (nrow(bouts) == 1) {
        ...
        table_of_bouts["medacc"] <- bouts$medacc
        table_of_bouts["medsd"] <- bouts$medsd
  ...
  all_columns <- unique(unlist(lapply(myfiles, names)))
  myfiles <- lapply(myfiles, function(x) {
    for (col in setdiff(all_columns, names(x))) {
      x[[col]] <- NA
    }
    return(x)
  })
  ...
  # Plot the results for the specific file
  plot <- ggplot(complete_file, aes(x = medacc, y = medsd, color = factor(Active))) +
    geom_point() +
    labs(title = paste("medacc vs medsd for", file_name_without_bin),
         x = "medacc",
         y = "medsd",
         color = "Activity") +  # Set legend title here
  ...
  # Save the plot
  plot_filepath <- file.path(specific_out_folder, paste(file_name_without_bin, "plot.png", sep = "_"))
  ggsave(plot_filepath, plot, width = 8, height = 8)
}

# Function to process all files and generate faceted plots
compute_for_all <- function(binfolder, outfolder) {
  ...
  plot <- ggplot(complete_file, aes(x = medacc, y = medsd, color = factor(Active))) +
    geom_point() +
    facet_wrap(~FILE, scales = "free") +  # Faceting by FILE column
    labs(title = "medacc vs medsd across all files",
         x = "medacc (g)",
         y = "medsd (g)",
         color = "Activity") +  # Set legend title here
    scale_color_discrete(labels = c("Sitting", "Standing")) +
  ...
  # Save the plot
  plot_filepath <- file.path(outfolder, "all_files_plot.png")
  ggsave(plot_filepath, plot, width = 16, height = 8)  # Adjust size as needed
}

# Run the processing for all files
compute_for_all("D:/Downloads/in", "D:/Downloads/out")

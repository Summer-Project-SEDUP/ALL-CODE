...
get_interval<-function(v_bin_data, binfile, start, end, calib, freq){
  ...
  #fill na this is used for edge cases in the window 
  non_wear_hours <- mean(rollapply(day_data, (10 * 60 * freq), non_wear_window, by = (freq * 60), by.column = FALSE, fill = NA)) * n_hours
...
create_ghost_table <- function(row, date) {
  ...
  #new
  ghost_tab <- data.frame(ghost_tab)
...
compute_one_person <- function(file_name, binfolder, outfolder) {
  ...
  for (i in (1:(length(filenames) - 1))) 
    ...
    if (nrow(D1$DataFrame) > 3000) {
      ...
      if (nrow(bouts) > 1) {
      ...
      else if (nrow(bouts) == 1) {
        #diff
        table_of_bouts <- as.data.frame(extract_metrics(ENMO_tab, freq))
        ...
        #table-of-bouts = data.frame(tsaberl of bouts)
...
compute_for_all("D:/Downloads/p004", "D:/Downloads/out/no-matrix")
...
# Function to detect non-wear periods
...
# Function to calculate sedentary and upright periods
...
get_raw_data <- function(D, bouts, freq) {
  nrows <- nrow(D)
  return(apply(bouts, 1, function(x) {
    start <- max(1, x[1])
    end <- min(nrows, x[2])
    if (start <= end) {
      D[start:end, ]
...
# Function to extract metrics from raw data
extract_metrics <- function(raw_bout, freq) {
  if (is.null(raw_bout)) return(NULL)
...
# Function to compute Windsorized standard deviation
...
create_ghost_table <- function(row, date) {
  ...
  times <- c("00:00:00", "01:00:00", "02:00:00", "03:00:00", "04:00:00", "05:00:00", "06:00:00", 
...
compute_one_person <- function(file_name, binfolder, outfolder) {
  ...
  for (i in (1:(length(filenames) - 1))) {
    ...
    if (nrow(D1$DataFrame) > 3000) {
      ...
      raw_bouts <- raw_bouts[!sapply(raw_bouts, is.null)] # Remove NULL elements
      if (length(raw_bouts) > 1) {
        list_of_bouts <- lapply(raw_bouts, extract_metrics, freq = freq)
        table_of_bouts <- do.call(rbind, lapply(list_of_bouts, as.data.frame))
      ...
      } else if (length(raw_bouts) == 1) {
        table_of_bouts <- extract_metrics(raw_bouts[[1]], freq)
...
compute_for_all("D:/Downloads/in", "D:/Downloads/out/direct")

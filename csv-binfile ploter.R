...
binfile <- "D:/Downloads/p004/P004_031202_2021-01-12 15-53-17.bin"
...
# Generate alternating color segments directly within the data
background_rects <- first_day_data %>%
  mutate(SegmentID = as.integer((Timestamp - min(Timestamp)) / (60 * 10))) %>% # Divide into 10-minute intervals
  group_by(SegmentID) %>%
  summarise(
    xmin = min(Timestamp),
    xmax = max(Timestamp),
...
plot_x <- ggplot(first_day_data) +
  geom_rect(data = background_rects,
            aes(xmin = xmin, xmax = xmax, ymin = -Inf, ymax = Inf, fill = "Activity Segment"),

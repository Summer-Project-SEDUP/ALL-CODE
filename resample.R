...
# Resampling Function
resample_data <- function(data, original_freq, target_freq) {
  time_index <- seq(from = 1, to = nrow(data)) / original_freq
  new_time_index <- seq(from = 1, to = tail(time_index, 1), by = 1 / target_freq)
  
  # Initialize an empty data frame for the resampled data
  resampled_data <- as.data.frame(matrix(nrow = length(new_time_index), ncol = ncol(data)))
  colnames(resampled_data) <- colnames(data)
  
  for (i in seq_along(colnames(data))) {
    resampled_data[, i] <- approx(time_index, data[, i], xout = new_time_index, rule = 2)$y
  }
  
  return(resampled_data)
}

# Get Interval Function with Resampling
get_interval <- function(v_bin_data, binfile, start, end, calib, original_freq, target_freq) {
  ...
  resampled_data <- resample_data(day_data, original_freq, target_freq)
  
  n_hours <- (nrow(resampled_data) / target_freq / 3600)
  non_wear_hours <- mean(rollapply(resampled_data, (10 * 60 * target_freq), non_wear_window, by = (target_freq * 60), by.column = FALSE, fill = NA)) * n_hours
  
  date <- as.Date(as.POSIXct(resampled_data[nrow(resampled_data) / 2, 1], origin = "1970-01-01"))
  day_of_week <- weekdays(date)
  
  return(list("DataFrame" = resampled_data, "Date" = date, "WeekDay" = day_of_week, "DayNo" = strsplit(start, " ")[[1]][1], "NonWear" = non_wear_hours))
}

# Non-Wear Window Function
...
compute_one_person <- function(file_name, binfolder, outfolder) {
  ...
  target_freq <- round(original_freq)  # Desired target frequency after resampling
  ...
  for (i in (1:(length(filenames) - 1))) {
    ...
    D1 <- get_interval(v_bin_data, binfile, start = start, end = end, calib = calib, original_freq = original_freq, target_freq = target_freq)
    
    if (nrow(D1$DataFrame) > 3000) {
      bouts <- compute_bouts(D1$DataFrame, target_freq)
      ENMO_tab <- add_ENMO(D1$DataFrame)
      raw_bouts <- get_raw_data(ENMO_tab, bouts, target_freq)
      
      if (nrow(bouts) > 1) {
        list_of_bouts <- lapply(raw_bouts, extract_metrics, freq = target_freq)
      ...
      if (nrow(bouts) == 1) {
        table_of_bouts <- extract_metrics(ENMO_tab, target_freq)
...
# Example Usage
compute_for_all("D:/Downloads/in", "D:/Downloads/out/resample")

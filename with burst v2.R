...
result <- compute_for_all("D:/Downloads/p004", "D:/Downloads/out/004")
...
measure_burstiness <- function(results, output_folder) {
  burstiness_results <- lapply(names(results), function(person) {
    person_dataframes <- results[[person]]
    person_burstiness <- lapply(names(person_dataframes), function(day) {
      ...
      # Convert each list item to a dataframe before processing
      df <- as.data.frame(df)
    ...
    # Save individual person burstiness metrics to CSV
    if (!is.null(person_burstiness) && nrow(person_burstiness) > 0) {
      write.csv(person_burstiness, file.path(output_folder, paste0(person, "_burstiness_metrics.csv")), row.names = FALSE)
    }
    
    return(person_burstiness)
  }) %>% bind_rows()
  
  # Save all burstiness metrics to a single CSV file
  write.csv(burstiness_results, file.path(output_folder, "all_burstiness_metrics.csv"), row.names = FALSE)
  
  return(burstiness_results)
}

# Example usage with your existing result data
output_folder <- "D:/Downloads/out/004/burstiness_output"  # Define the output folder for CSV files
dir.create(output_folder, showWarnings = FALSE)  # Create the folder if it doesn't exist

burstiness_metrics <- measure_burstiness(result, output_folder)

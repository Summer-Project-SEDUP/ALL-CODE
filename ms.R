...
get_interval <- function(v_bin_data, binfile, start, end, calib, freq) {
  ...
  n_hours <- (nrow(day_data) / (freq * 1000 * 3600))  # Adjusted for milliseconds
  non_wear_hours <- mean(rollapply(day_data, (10 * 60 * freq * 1000), non_wear_window, by = (freq * 60 * 1000), by.column = FALSE)) * n_hours
  date <- as.Date(as.POSIXct(day_data[nrow(day_data)/2, 1] / 1000, origin = "1970-01-01"))  # Adjusted for milliseconds
...
extract_metrics <- function(raw_bout, freq) {
  volume <- sum(raw_bout[,"ENMO"]) / (freq * 1000)  # Adjusted for milliseconds
  intensity <- mean(raw_bout[,"ENMO"])
  timestamp <- strftime(format(as.POSIXct(raw_bout[1, 1] / 1000, origin = "1970-01-01"), tz = "UTC"), format = "%H:%M:%S")  # Adjusted for milliseconds
  duration <- nrow(raw_bout) / (freq * 1000)  # Adjusted for milliseconds
...
compute_one_person <- function(file_name, binfolder, outfolder) {
  ...
  dates <- c(seq(d1, d2, by = 1000 * 3600 * 24), d2)  # Adjusted for milliseconds
...
compute_for_all("D:/Downloads/in", "D:/Downloads/out/ms")

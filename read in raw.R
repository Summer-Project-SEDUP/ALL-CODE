...
extract_raw_data <- function(bin_file_path) {
  ...
  repeat {
    ...
    time_stamp_chunk <- readBin(con, "numeric", size = 8, n = chunk_size, endian = "little")
    if (length(time_stamp_chunk) == 0) break  # End of file reached
    
    x_axis_chunk <- readBin(con, "numeric", size = 8, n = chunk_size, endian = "little")
    y_axis_chunk <- readBin(con, "numeric", size = 8, n = chunk_size, endian = "little")
    z_axis_chunk <- readBin(con, "numeric", size = 8, n = chunk_size, endian = "little")
    light_level_chunk <- readBin(con, "numeric", size = 8, n = chunk_size, endian = "little")
    button_chunk <- readBin(con, "integer", size = 1, n = chunk_size, signed = TRUE)
    temperature_chunk <- readBin(con, "numeric", size = 8, n = chunk_size, endian = "little")
